...
/*###############################################################################################################################################*/
int answer(string &alpha, int start, string &beta)
{
    char initial1 = alpha[start];
    char prev = initial1;
    int count_1 = 0;
    while (start < alpha.length() && (alpha[start] == prev))
    {
        count_1++;
        start++;
    }
    prev = alpha[start];
    beta.push_back(initial1);
    beta.push_back(initial1);
    if (start < alpha.length() && alpha[start] == alpha[start + 1])
    {
        int count_2 = 0;
        char initial2 = alpha[start];
        while (start < alpha.length() && alpha[start] == prev)
        {
            count_2++;
            start++;
        }
        beta.push_back(initial2);
    }
    return start;
}
int main()
{
    ...
    while (index < n)
    {
        if ((index + 1 < n) && (alpha[index] == alpha[index + 1]))
        {
            int p1 = answer(alpha, index, final_answer);
            index = p1;
        }
        else
        {
            final_answer.push_back(alpha[index]);
            index++;
        }
    }
    cout << final_answer << "\n";
